...
class Pos_invoices extends CI_Controller
{
    ...
    public function action()
    {
        ...
        $refer = $this->input->post('refer', true);
          $notes_invoices = $this->input->post('notes_invoice', true);
          $change_amount = rev_amountExchange_s($this->input->post('change_p'), $currency, $this->aauth->get_user()->loc);;
        ...
        if ($d_trans['key2']) {
           ...
           $t_data['type'] = 'Expense';
            ...
            $data = array('tid' => $invocieno, 'invoicedate' => $bill_date, 'invoiceduedate' => $bill_due_date, 'subtotal' => $subtotal, 'shipping' => $shipping, 'total' => $total, 'notes' => $notes, 'csd' => $customer_id, 'eid' => $emp, 'taxstatus' => $tax, 'discstatus' => $discstatus, 'format_discount' => $discountFormat, 'refer' => $refer, 'notes_invoice' => $notes_invoices,'change_amount' => $change_amount,'amount' => $p_amount, 'term' => $pterms, 'multi' => $currency, 'i_class' => 1, 'loc' => $this->aauth->get_user()->loc);
            $invoice_true = false;
            if ($this->db->insert('geopos_invoices', $data)) {
                $invoice_true = true;
                $tid = $invocieno;
