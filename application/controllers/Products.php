...
<div class="dropdown-divider"></div>&nbsp;<a href="' . base_url() . 'products/duplicate?id=' . $pid . '" class="btn btn-primary btn-sm"><span class="fa fa-plus"></span>' . " Duplicate" . '</a>
                                    </div>
                                </div>';
            $data[] = $row;
        }
    ...
    public function delete_i()
 {
    ...
    public function duplicate_i()
 {
        if ( $this->aauth->premission( 11 ) ) {
            $id = $this->input->post( 'duplicateid' );
            if ( $id ) {
              $whr = 'WHERE pid=' . $id;

              $this->db->query("insert into geopos_products (pcat, warehouse, product_name, product_code,product_price,fproduct_price,taxrate, disrate, qty, product_des,alert,unit,image,barcode,merge,sub,vb,expiry,code_type,sub_id,b_id)
select pcat, warehouse, product_name, product_code,product_price,fproduct_price,taxrate, disrate, qty, product_des,alert,unit,image,barcode,merge,sub,vb,expiry,code_type,sub_id,b_id
from geopos_products $whr");

                echo json_encode( array( 'status' => 'Success', 'message' => "The entry has been duplicated Successfully!" ) );
            } else {
                echo json_encode( array( 'status' => 'Error', 'message' => $this->lang->line( 'ERROR' ) ) );
            }
        } else {
    ...
    public function duplicate()
 {
        ...
        $data['variables'] = $this->units->variables_list();
        $this->load->view( 'fixed/header', $head );
        $this->load->view( 'products/product-duplicate', $data );
        $this->load->view( 'fixed/footer' );
 ...
 {
        $pid = $this->input->get( 'id' );
        $query = $this->db->query( "SELECT product_name,barcode,code_type FROM geopos_products WHERE pid='$pid'" );
        $result = $query->row_array();
        $data['name'] = $result['product_name'];
        $data['code'] = $result['barcode'];
        $data['ctype'] = $result['code_type'];
        $html = $this->load->view( 'barcode/view', $data );
        ...
        if($data['name']) $pdf->WriteHTML($html);
