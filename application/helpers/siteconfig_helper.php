...
function amountExchange_label( $number, $id = 0, $loc = 0 )
 {
    $ci = & get_instance();
    $ci->load->database();
    if ( $loc > 0 && $id == 0 ) {
    ...
    if ( $id > 0 ) {
        ...
        //Format money as per country
        if ( $row['cpos'] == 0 ) {
            return "Rp " . @number_format( $totalamount, $decimal_after, $dec_point, $thosand );
        } else {
            return @number_format( $totalamount, $decimal_after, $dec_point, $thosand ) . ' ' . $currency;
        }
    } else {
        ...
        //Format money as per country
        if ( $row['method'] == 'l' ) {
            return "Rp " . @number_format( $number, $row['url'], $row['key1'], $row['key2'] );
        } else {
            return @number_format( $number, $row['url'], $row['key1'], $row['key2'] ) . ' ' . $currency;
        }
    }
...
function amountExchange_not_point( $number, $id = 0, $loc = 0 )
 {
    $ci = & get_instance();
    $ci->load->database();
    if ( $loc > 0 && $id == 0 ) {
    ...
    if ( $id > 0 ) {
        ...
        //Format money as per country
        if ( $row['cpos'] == 0 ) {
            return $currency . @number_format( $totalamount );
        } else {
            return @number_format( $totalamount, $decimal_after, $dec_point, $thosand ) . ' ' . $currency;
        }
    } else {
...
function amountExchange_new( $number, $id = 0, $loc = 0 )
 {
    $ci = & get_instance();
    $ci->load->database();
    if ( $loc > 0 && $id == 0 ) {
