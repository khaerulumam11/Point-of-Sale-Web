            ...
            <form method="post" id="basicpay_data" class="form-horizontal">
                ...
                <div class="modal-body">
                        ...
                        <div class="col-6">
                            <div class="form-group text-bold-600 text-g">
                                ...
                                <input
                                        ...
                                        name="change_p" id="change_p" value="0">
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group text-bold-600 text-g">
                                <label for="invocieno">Reference Number/ Rekening Number</label>
                                <input
                                        type="text"
                                        class="form-control green"
                                        name="refer" id="refer">
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="form-group text-bold-600 text-g">
                                <label for="b_change">Note</label>
                                <textarea name="notes_invoice" id="notes_invoice" rows="8" cols="80" class="form-control green"></textarea>
...
<script type="text/javascript">
    ...
    $('#pos_basic_pay').on("click", function (e) {
        ...
        $("#notify").removeClass("alert-danger").addClass("alert-primary").fadeIn();
        $("html, body").animate({scrollTop: $('body').offset().top - 100}, 1000);
        var o_data = $("#data_form").serialize() + '&p_amount=' + accounting.unformat($('#p_amount').val(), accounting.settings.number.decimal) + '&p_method=' + $("#p_method option:selected").val() + '&type=' + $(this).attr('data-type') + '&account=' + $("#p_account option:selected").val() + '&employee=' + $("#employee option:selected").val() + '&refer=' + $('#refer').val() + '&notes_invoice=' + $('#notes_invoice').val() + '&p_amount=' + $('#p_amount').val() + '&change_p=' + $('#change_p').val();
        var action_url = $('#action-url').val();
        addObject(o_data, action_url);
        setTimeout(
            function () {
            ...
            if (!$(this).attr('readonly')) {
                ...
                if (this.value.length > 2) {
                    $.ajax({
                        type: "POST",
                        url: baseurl + 'search_products/v2_pos_search',
                        data: 'name=' + $(this).val() + '&wid=' + whr + '&cid=' + cat + '&' + crsf_token + '=' + crsf_hash +'&bid=' + $(this).val() +'&bar=' + $('#bar_only').prop('checked'),
                        beforeSend: function () {
                            $("#customer-box").css("background", "#FFF url(" + baseurl + "assets/custom/load-ring.gif) no-repeat 165px");
                        },
                        success: function (data) {
